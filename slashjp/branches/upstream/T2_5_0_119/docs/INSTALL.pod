...
previous versions of Slash, see the INSTALL document included with
those distributions.
...
you probably do not want to use the very latest CVS, but you probably
do want to use the latest "R_" tag available in CVS.  See L<"VERSIONS">,
"CVS tags", below, for advice on choosing and maintaining a CVS installation.
...
hardware requirements are given later on in the L<"REQUIREMENTS">
section, which you probably want to go read now, to check for potential
problems.
...
B<IMPORTANT NOTES (read through these first)>:
...
optional but, if there are problems with these modules, you'll want to
resolve them before moving on to the rest of the installation:
...
After the LWP has been installed and configured successfully, C<exit>
the CPAN, reinvoke it with the C<perl -MCPAN -e shell> command, and
...
Don't worry about the threading warning.
...
Apache::Test's self-tests are a colossal pain to actually run.
Personally I just C<force install Apache::Cookie> which is lame but
solves the problem.
...
but if you're down to 1 or 2 you can't fix, we suggest you make a note
...
If you are feeling lucky and are in a big hurry, you might try C<force
install Bundle::Slash>, which will skip all tests for all modules --
but if you do this and later things start to fail, you're on your own,
don't complain.
...
Memcached is not required, but more and more, we are adding optimizations
to Slash which take load off your database.  If you are concerned about
performance, this is one of the first options to install.
...
you download them using the CPAN module.  (See L<"INSTALLATION">,
item 4, "Install the Perl modules.")
...
a small database -- even Slashdot.org's DB compresses down to under 2 GB.
...
R_2_3_0_x cvs tag ("Release candidates for 2.3.0").
...
$Id: INSTALL.pod,v 1.52 2006/02/26 21:30:39 jamiemccarthy Exp $
