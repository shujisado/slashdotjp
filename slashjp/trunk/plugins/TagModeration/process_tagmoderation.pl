...
$task{$me}{code} = sub {
	...
	if (!$constants->{m1} || $constants->{m1_pluginname} ne 'TagModeration') {
		slashdLog("$me - TagModeration inactive") if verbosity() >= 2;
		return ;
	}
...
sub give_out_points {
	...
	if ($newcomments > 0) {
		...
		my $granted = $tagmoddb->convert_tokens_to_points($needed);
...
sub give_out_tokens {
	...
	my $stirredpoints = $tagmoddb->stirPool();
	...
	$tagmoddb->updateTokens(\%update_uids, { sleep_time => $sleep_time });
...
sub delete_old_mod_rows {
	my $tagmoddb = getObject('Slash::TagModeration');
	$tagmoddb->deleteOldModRows({ sleep_between => 30 });
}
